...
export class OrdersService {
  ...
  hasMap: flags.map,
      ...
      referenceReviews: dto.referenceText || '',
        ...
        videoCount: Math.max(0, Math.min(10, Math.trunc(Number(dto.videoCount ?? 0)))), // V2: 동영상 플레이스홀더
        // V2: 대행사가 주문 시 페르소나 선택
        personaId: dto.personaId || null,
        personaSnapshot: dto.personaSnapshot || null,
      });
      orders.push(order);
    }
    ...
    // V2: SUBMITTED 주문은 즉시 생성 큐에 넣는다 (admin intake 불필요)
    // 페르소나가 없으면 기본값 자동 할당
    if (!saveAsDraft) {
      for (const order of created) {
        if (!String(order.personaSnapshot || '').trim()) {
          order.personaId = '30-F-FRIENDLY-BRIGHT';
          order.personaSnapshot = '30대 여성, 친근하고 밝은 톤의 작성 스타일';
          await this.orderRepository.save(order);
          console.log(`[OrdersService] Auto-assigned default persona for order ${order.id}`);
        }
        try {
          await this.generationService.enqueueGenerate(order.id);
          console.log(`[OrdersService] Auto-enqueue generation for order ${order.id}`);
        } catch (err: any) {
          console.error(`[OrdersService] Auto-enqueue failed for order ${order.id}:`, err.message);
        }
      }
    }
