...
export class AdminService {
  ...
  async assignPersona(id: string, assignDto: AssignPersonaDto): Promise<Order> {
    ...
    // V2: SUBMITTED(대행사가 페르소나 안 넣은 경우)도 허용, ADMIN_INTAKE도 재배정 허용
    if (order.status !== OrderStatus.SUBMITTED && order.status !== OrderStatus.ADMIN_INTAKE) {
      throw new ForbiddenException('Order must be in SUBMITTED or ADMIN_INTAKE status');
    }
