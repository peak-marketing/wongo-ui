...
import { buildCaptionPrompt, buildCorrectionPrompt, buildManuscriptPrompt, buildPhotoSortPrompt, buildReceiptReviewPrompt, CaptionItem } from './prompt.templates';
import { openaiGenerateContent, registerOpenAIConcurrencyReducer } from '../ai/openai.client';
...
const getWorkerConcurrency = () => {
  return Math.max(
    1,
    toPosInt(process.env.GEN_WORKER_CONCURRENCY, toPosInt(process.env.GEN_CONCURRENCY, toPosInt(process.env.OPENAI_CONCURRENCY, 5))),
  );
};
...
export class GenerationProcessor extends WorkerHost {
  ...
  onModuleInit() {
    registerOpenAIConcurrencyReducer(async ({ prev, next, reason, at }) => {
  ...
  private pickPrimaryModel(mode?: 'speed' | 'quality', qualityMode?: boolean): string {
    const baseModel = String(process.env.OPENAI_MODEL_BASE || 'gpt-4o-mini').trim();
    const proModel = String(process.env.OPENAI_MODEL_PRO || 'gpt-4o').trim();
  ...
  private pickPrimaryModelForOrder(args: { orderType?: OrderType; mode?: 'speed' | 'quality'; qualityMode?: boolean }): string {
    ...
    // MANUSCRIPT speed 모델은 별도 매핑. 설정 없으면 baseModel 사용.
    const manuscriptSpeedModel = String(process.env.OPENAI_MODEL_MANUSCRIPT_SPEED || '').trim();

    // RECEIPT_REVIEW speed 모델은 별도 매핑(환경변수로 조정 가능)
    const receiptSpeedModel = String(process.env.OPENAI_MODEL_RECEIPT_SPEED || '').trim();
  ...
  private async callOpenAIGenerateContent(args: {
    orderId: string;
    ...
    try {
      ...
      console.log(
        `(${new Date().toISOString()}) (OPENAI_MODEL_USED) (SUCCESS) (${args.orderId}|${result.model}|attempts=${result.attemptsUsed})`,
      );
      return result.text;
    } catch (e: any) {
      console.log(
        `(${new Date().toISOString()}) (OPENAI_MODEL_USED) (FAIL) (${args.orderId}|${model}|status=${this.extractStatusCode(e)} )`,
      );
      throw e;
    }
  }

  private async callOpenAIGenerateText(args: {
    orderId: string;
    prompt: string;
    orderType?: OrderType;
    mode?: 'speed' | 'quality';
    ...
    return this.callOpenAIGenerateContent({
  ...
  private normalizeManuscript(text: string): string {
    ...
    // 3.45) strip meta-narrative sentences (repetitive ending pattern)
    // Removes lines like: "~기준으로 디테일을 더 짚어보면 사진 흐름이 자연스럽게 이어져요"
    // and similar bot-generated meta descriptions about photo flow/arrangement
    t = t
      .split('\n')
      .filter((line) => {
        const l = line.trim();
        // pattern: "X와/과 Y 기준으로 디테일을 더 짚어보면 사진 흐름이 자연스럽게 이어져요"
        if (/기준으로\s*디테일을\s*더\s*짚어보면/.test(l)) return false;
        if (/사진\s*흐름이\s*자연스럽게\s*이어져요/.test(l)) return false;
        // pattern: "~와/과 ~ 기준으로 ~ 이어져요/이어집니다" (broader catch)
        if (/[가-힣]+[와과]\s+[가-힣]+\s+기준으로/.test(l) && /이어져요|이어집니다|이어지네요/.test(l)) return false;
        // pattern: "디테일을 더 짚어보면" standalone
        if (/디테일을\s*더\s*짚어보면/.test(l) && l.length < 80) return false;
        return true;
      })
      .join('\n');
  ...
  private buildHashtagsLine(args: {
    ...
    // V2 정책: 사용자가 해시태그를 0개 입력했으면 fallback 자동채움 없이
    // AI가 프롬프트에서 적절히 생성하도록 빈 상태로 유지한다.
    // 사용자가 1~5개 입력했으면 그것만 사용.
    if (tags.length === 0) {
      return { line: '', tags: [] };
    }
  ...
  private validateManuscript(args: {
    ...
    // V2: 강조 키워드는 최소 2회 포함 필수 + 과다 반복(5회 초과) 경고
    for (const kw of emphasis) {
      if (!kw) continue;
      const count = this.countOccurrences(text, kw);
      if (count === 0) {
        failures.push(`강조 키워드 누락: ${kw}`);
      } else if (count < 2) {
        failures.push(`강조 키워드 반복 부족: ${kw} (최소 2회 필요, 현재=${count})`);
      } else if (count > 5) {
        failures.push(`강조 키워드 과다 반복: ${kw} (최대 5회 권장, 현재=${count})`);
      }
    }
    // 필수 키워드도 과다 반복 체크
    for (const kw of required) {
      ...
      const count = this.countOccurrences(text, kw);
      if (count > 5) {
        failures.push(`필수 키워드 과다 반복: ${kw} (최대 5회 권장, 현재=${count})`);
      }
    }
  ...
  async process(job: Job<GenerationJobData>): Promise<void> {
    ...
    const runOnce = async () => {
      if (order.type === OrderType.RECEIPT_REVIEW) {
        ...
        for (let i = 1; i <= outputCount; i++) {
          ...
          for (let attempt = 1; attempt <= 3; attempt++) {
            ...
            const raw = await this.callOpenAIGenerateContent({ orderId, parts, orderType: order.type, mode: effectiveMode, qualityMode, modelOverride });
      ...
      for (let i = 1; i <= photos.length; i++) {
        ...
        const captionRaw = await this.callOpenAIGenerateContent({
          orderId,
      ...
      // Step A-2: 사진 자동 정렬 (블로그 흐름 최적화)
      let sortedCaptions = captions;
      let sortedPhotoLabels = photoLabels;
      let sortedPhotos = photos;
      try {
        const sortPrompt = buildPhotoSortPrompt({
          placeName: order.placeName,
          placeAddress: order.placeAddress,
          searchKeywords: order.searchKeywords,
          captions,
        });
        const sortRaw = await this.callOpenAIGenerateText({
          orderId,
          prompt: sortPrompt,
          orderType: order.type,
          mode: effectiveMode,
        ...
        // JSON 배열 파싱
        const cleaned = sortRaw.replace(/```[\s\S]*?```/g, '').trim();
        const arrMatch = cleaned.match(/\[([\d,\s]+)\]/);
        if (arrMatch) {
          const sortedIndices: number[] = JSON.parse(arrMatch[0]);
          // 검증: 모든 원본 index가 포함되어야 함
          const allOriginal = captions.map(c => c.index).sort((a, b) => a - b);
          const allSorted = [...sortedIndices].sort((a, b) => a - b);
          if (JSON.stringify(allOriginal) === JSON.stringify(allSorted)) {
            // 정렬 적용: captions, photoLabels, photos 모두 재배열
            sortedCaptions = sortedIndices.map((origIdx, newPos) => {
              const orig = captions.find(c => c.index === origIdx)!;
              return { ...orig, index: newPos + 1 };
            });
            sortedPhotoLabels = sortedIndices.map(origIdx => photoLabels[origIdx - 1]);
            sortedPhotos = sortedIndices.map(origIdx => photos[origIdx - 1]);
            console.log(
              `${new Date().toISOString()} (PHOTO_SORT) (${orderId}|original=${captions.map(c=>c.index).join(',')}|sorted=${sortedIndices.join(',')})`,
            );
          } else {
            console.log(
              `${new Date().toISOString()} (PHOTO_SORT) (SKIP_MISMATCH) (${orderId}|expected=${allOriginal.join(',')}|got=${allSorted.join(',')})`,
            );
          }
        } else {
          console.log(
            `${new Date().toISOString()} (PHOTO_SORT) (SKIP_PARSE) (${orderId}|raw=${sortRaw.substring(0, 200)})`,
          );
        }
      } catch (sortErr: any) {
        // 정렬 실패 시 원본 순서로 계속 진행 (실패가 전체 생성을 막지 않음)
        console.log(
          `${new Date().toISOString()} (PHOTO_SORT) (ERROR) (${orderId}|err=${sortErr.message})`,
        );
      }

      // Step B: manuscript (정렬된 순서 사용)
      ...
      console.log(
        `${new Date().toISOString()} (OPENAI_PROMPT) (${orderId}|promptChars=${manuscriptPrompt.length}|revisionChars=${revisionText.length}|revisionHash=${revisionText ? this.hash8(revisionText) : '0'}|extraChars=${extraText.length}|extraHash=${extraText ? this.hash8(extraText) : '0'}|photos=${sortedPhotoLabels.length})`,
      );

      let manuscript = await this.callOpenAIGenerateText({ orderId, prompt: manuscriptPrompt, orderType: order.type, mode: effectiveMode, qualityMode });
      ...
      const autoFix1 = this.autoFixBeforeFinalValidate({
        manuscript,
        photoCount: sortedPhotoLabels.length,
        captions: (sortedCaptions || []).map((c: any) => ({ index: c.index, caption: c.caption, tags: c.tags, ocr: c.ocr })),
        requiredKeywords,
        emphasisKeywords,
        placeName: order.placeName,
        searchKeywords: order.searchKeywords || '',
      ...
      if (!validation1.ok) {
        ...
        manuscript = await this.callOpenAIGenerateText({ orderId, prompt: correctionPrompt, orderType: order.type, mode: effectiveMode, qualityMode });
      ...
      // V2: 동영상 플레이스홀더 append
      const videoCount = Math.max(0, Math.trunc(Number((order as any).videoCount ?? 0)));
      if (videoCount > 0) {
        const videoLines: string[] = [];
        for (let v = 1; v <= videoCount; v++) {
          videoLines.push(`동영상 ${v}`);
        }
        // 해시태그 라인 앞에 삽입 (해시태그가 없으면 맨 끝)
        const msLines = manuscript.replace(/\r\n?/g, '\n').split('\n');
        const hashtagIdx = msLines.findIndex((l) => /^해시태그:\s*/.test(l.trim()));
        if (hashtagIdx >= 0) {
          msLines.splice(hashtagIdx, 0, '', ...videoLines, '');
        } else {
          msLines.push('', ...videoLines);
        }
        manuscript = msLines.join('\n').replace(/\n{3,}/g, '\n\n').trim();
      }

      // V2: 사진별 매칭 근거(caption/tags)를 payload에 저장하여 프론트에서 검증 가능
      const existingPayload = (order as any).payload && typeof (order as any).payload === 'object' ? (order as any).payload : {};
      const v2Payload = {
        ...existingPayload,
        photoCaptions: sortedCaptions.map((c) => ({
          index: c.index,
          caption: c.caption,
          tags: c.tags,
          ocr: c.ocr || [],
      ...
      // 정렬된 사진 순서도 저장 (photos 배열 업데이트)
      const updateData: any = {
        status: OrderStatus.GENERATED,
        manuscript,
        payload: v2Payload as any,
        lastFailureReason: null,
      };
      // 정렬이 적용된 경우 photos 순서도 업데이트
      if (sortedPhotos !== photos) {
        updateData.photos = sortedPhotos;
      }
