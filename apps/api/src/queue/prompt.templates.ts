...
export function buildManuscriptPrompt(args: {
  ...
  return safeJoin([
    ...
    '- (중요) 각 사진 블록의 본문은 문장 줄바꿈 기준으로 4줄(±1줄) 정도로 간결하게 쓴다(너무 길게 늘어지지 않게).',
    ...
    `1) 1번째 줄: 제목: {${args.placeName} 포함, 검색 키워드(${args.searchKeywords || '없음'}) 중 1개 이상을 자연스럽게 포함, 25~40자}`, 
    '2) 2번째 줄: 빈 줄',
    ...
    '     b-1) 본문은 문장 줄바꿈 기준으로 4줄(±1줄) 정도로 간결하게 작성한다.',
    ...
    '- (절대 금지) 각 사진 블록의 마지막 문장이 동일하거나 유사한 구조로 반복되면 실패다. 20개 블록이면 20가지 서로 다른 마무리를 써야 한다.',
    '- (절대 금지) 사진 배치/구성/흐름을 설명하는 메타 서술은 본문에 쓰지 않는다. 독자에게 유용한 실제 정보·감상·꿀팁만 쓴다.',
    '- (절대 금지) "A와 B 기준으로 ~" 같은 태그 나열식 문장은 쓰지 않는다.',
    '- 각 사진 블록은 서로 다른 어투·표현·리듬으로 마무리한다. 아래 마무리 변주 예시를 참고하되 그대로 복사하지 마라:',
    '  · 감탄/추천형: "다음에 또 오고 싶은 곳이에요!"',
    '  · 꿀팁 제공형: "점심시간 전에 가면 웨이팅 없이 바로 앉을 수 있어요."',
    '  · 감각 묘사형: "고소한 향이 입안 가득 퍼지는 느낌이랄까요."',
    '  · 비교/강조형: "이 가격에 이 퀄리티는 정말 흔치 않아요."',
    '  · 정보 전달형: "주차장이 넓어서 차 가져가기 편해요."',
    '  · 분위기 전달형: "조명이 은은해서 데이트 장소로도 딱이에요."',
    '- 필수 키워드는 원문 그대로 본문(body)에 최소 1회, 최대 5회 포함한다.',
    '- 강조 키워드는 원문 그대로 본문(body)에 최소 2회, 최대 5회 포함한다.',
    '- 키워드를 한 문단에 몰아 넣지 말고 여러 문단에 분산 배치한다.',
    '- 본문(body) 글자수는 1500~2300자 범위를 맞춘다(제목/해시태그 제외).',
    '',
    '[해시태그 생성 규칙]',
    '- 입력 해시태그가 있으면 반드시 그대로 포함하고, 총 1~5개 범위로만 맞춘다.',
    '- 입력 해시태그가 없으면(0개), 업체명/검색키워드/본문 맥락에서 적절한 해시태그 3~5개를 직접 생성한다.',
    '- 해시태그는 과다 생성하지 않는다. 총 5개를 넘기지 않는다.',
    ...
    captionsJson,
    '',
    '[출력 예시(형식 참고용 — 내용은 실제 정보로 작성, 마무리 문장이 모두 다름에 주목)]',
    '제목: {placeName 포함 25~40자}',
    '',
    '사진 1',
    '가게 외관이 깔끔하게 정리되어 있어요.',
    '간판도 크게 보여서 찾기 정말 쉬웠어요.',
    '주차 공간도 넉넉해서 차로 방문하기 좋아요.',
    '',
    '사진 2',
    '내부는 테이블 간격이 넓어서 편하게 식사할 수 있어요.',
    '단체 모임이나 가족 외식에도 잘 어울리는 공간이에요.',
    '조명이 은은해서 분위기까지 챙길 수 있답니다.',
    '',
    '사진 3',
    '대표 메뉴가 나오자마자 비주얼에 감탄했어요.',
    '양도 푸짐하고 가성비가 정말 좋아요.',
    '이 가격에 이 퀄리티는 주변에서 찾기 어려울 것 같아요.',
    '',
    '...',
    `사진 ${photoCount}`,
    '마지막으로 디저트와 음료까지 깔끔하게 마무리했어요.',
    '다음에 또 방문하고 싶은 곳이에요!',
    '',
    '해시태그: #... #... #...',
  ]);
...
export function buildPhotoSortPrompt(args: {
  placeName: string;
  placeAddress?: string;
  searchKeywords?: string;
  captions: CaptionItem[];
}): string {
  const captionsSummary = args.captions.map(c => ({
    index: c.index,
    caption: c.caption,
    tags: c.tags,
    ocr: c.ocr,
  ...
  return safeJoin([
    '너는 한국어 블로그 포스팅 전문가다. 아래 사진 측션 목록을 보고, 블로그 포스팅에 가장 자연스러운 순서로 사진을 정렬해라.',
    ...
    '',
    '[정렬 규칙]',
    '- 블로그 포스팅의 자연스러운 흐름에 맞게 정렬한다.',
    '- 권장 순서: 외관/간판 → 내부/인테리어 → 메뉴판/이벤트/특징 → 음식(메인) → 음식(디테일/반찬) → 마무리(디저트/음료/남은음식)',
    '- 같은 카테고리 내에서는 원래 순서를 유지한다.',
    '- 모든 사진을 빠짐없이 포함한다(삭제 금지).',
    '- 사진이 어느 카테고리에도 명확히 속하지 않으면, 가장 가까운 카테고리에 배치한다.',
    '',
    '[사진 측션 JSON]',
    JSON.stringify(captionsSummary, null, 2),
    '',
    '[출력 형식]',
    '정렬된 사진 index를 순서대로 담은 JSON 배열만 출력한다.',
    '예: [3, 1, 5, 2, 4, 6, 7, 8, 9, 10]',
  ]);
}
...
export function buildCorrectionPrompt(args: {
  ...
  return safeJoin([
    ...
    '- (중요) 각 사진 블록의 본문은 문장 줄바꿈 기준으로 4줄(±1줄) 정도로 간결하게 보정한다.',
    '- 사진 블록과 블록 사이는 빈 줄 1줄(\n\n)로만 구분한다.',
    '- 해시태그 라인은 반드시 마지막 줄 1개이며, 그 뒤에 어떤 텍스트도 출력하지 않는다.',
    '- 지도/링크는 원고 본문에 직접 쓰지 않는다(서버가 최종 append). 단, 서버가 붙이는 지도 URL 라인은 반드시 "지도 삽입 : {URL}" 형식이다.',
    ...
    '',
    '[출력 계약(Contract) - 절대 준수]',
    `1) 1번째 줄: 제목: {placeName 포함, 검색 키워드(${args.searchKeywords || '없음'}) 중 1개 이상을 자연스럽게 포함, 25~40자}`, 
    '2) 2번째 줄: 빈 줄',
    ...
    `- 사진 순서는 1→${photoCount} 고정(역순/점프/누락 금지).`,
    '- (절대 금지) 각 사진 블록의 마지막 문장이 동일하거나 유사한 구조로 반복되면 실패다.',
    '- (절대 금지) 사진 배치/구성/흐름을 설명하는 메타 서술은 본문에 쓰지 않는다.',
    '- (절대 금지) "A와 B 기준으로 ~" 같은 태그 나열식 문장은 쓰지 않는다.',
    '- 각 사진 블록은 서로 다른 어투·표현·리듬으로 마무리한다(복사-붙여넣기식 변형 금지).',
