...
export class AgencyStatsController {
  ...
  async getStats(@GetUser() user: any) {
    ...
    // V2: 원고완료 버킷 (GENERATED + AGENCY_REVIEW)
    const manuscriptReadyStatuses = new Set<OrderStatus>([
    ...
    let writing = 0;
    let manuscriptReady = 0;
    let todayDone = 0;

    orders.forEach((order) => {
      const status = order.status as OrderStatus;

      // V2: GENERATED / AGENCY_REVIEW → 원고완료 버킷
      if (manuscriptReadyStatuses.has(status)) {
        manuscriptReady += 1;
        return;
      }
    ...
    return {
      writing,
      manuscriptReady,
      // 하위호환: 기존 firstReview 필드도 유지
      firstReview: manuscriptReady,
      todayDone,
