...
export class AgencyService {
  ...
  /**
   * V2: 파일명 = '대표키워드_업체명.zip'
   * Windows 금지문자 제거, 길이 제한(80자), fallback → orderId
   */
  private getOrderDeliverableZipFileName(orderId: string, order?: { placeName?: string; searchKeywords?: string }) {
    if (order) {
      const place = String(order.placeName || '').trim();
      const firstKw = String(order.searchKeywords || '')
        .split(/[,\n\t]/)
        .map((s) => s.trim())
        .filter(Boolean)[0] || '';
      const raw = firstKw && place ? `${firstKw}_${place}` : place || firstKw || '';
      // Windows 금지문자 + 공백 정리
      const safe = raw
        .replace(/[<>:"\\|?*\/]/g, '')
        .replace(/\s+/g, ' ')
        .trim()
        .slice(0, 80);
      if (safe.length > 0) return `${safe}.zip`;
    }
  ...
  async createDeliverableZipStream(orderId: string, userId: string): Promise<{ stream: Readable; fileName: string }> {
    ...
    const fileName = this.getOrderDeliverableZipFileName(orderId, order);
  ...
  async findAll(
    ...
    } else if (bucket === 'firstReview' || bucket === 'manuscriptReady') {
      // V2: GENERATED + AGENCY_REVIEW 둘 다 원고완료 버킷
      qb.andWhere('order.status IN (:...statuses)', { statuses: [OrderStatus.GENERATED, OrderStatus.AGENCY_REVIEW] });
  ...
  async submit(id: string, userId: string): Promise<Order> {
    ...
    // V2: SUBMITTED 주문은 즉시 생성 큐에 넣는다
    // 페르소나가 없으면 기본값 자동 할당
    if (!String(saved.personaSnapshot || '').trim()) {
      saved.personaId = '30-F-FRIENDLY-BRIGHT';
      saved.personaSnapshot = '30대 여성, 친근하고 밝은 톤의 작성 스타일';
      await this.orderRepository.save(saved);
      console.log(`[AgencyService.submit] Auto-assigned default persona for order ${saved.id}`);
    }
    try {
      await this.generationService.enqueueGenerate(saved.id);
      console.log(`[AgencyService.submit] Auto-enqueue generation for order ${saved.id}`);
    } catch (err: any) {
      console.error(`[AgencyService.submit] Auto-enqueue failed for order ${saved.id}:`, err.message);
    }
  ...
  async review(id: string, userId: string, reviewDto: ReviewOrderDto): Promise<Order> {
    ...
    if (reviewDto.decision === ReviewDecision.APPROVE) {
      ...
      // V2: GENERATED 또는 AGENCY_REVIEW 상태에서 승인 가능
      if (order.status !== OrderStatus.AGENCY_REVIEW && order.status !== OrderStatus.GENERATED) {
      ...
      try {
        const saved = await this.dataSource.transaction(async (manager) => {
          ...
          // V2: GENERATED 또는 AGENCY_REVIEW 둘 다 허용
          if (lockedOrder.status !== OrderStatus.AGENCY_REVIEW && lockedOrder.status !== OrderStatus.GENERATED) {
            throw new ConflictException('Order status changed; review not allowed');
          }
      ...
      // V2: GENERATED 또는 AGENCY_REVIEW에서 수정요청 가능
      if (order.status !== OrderStatus.AGENCY_REVIEW && order.status !== OrderStatus.GENERATED) {
        throw new ConflictException('Order status changed; review not allowed');
      }
  ...
  sanitizeForAgency(order: Order): any {
    ...
    // V2: GENERATED 이후부터 원고 표시 (영업자가 바로 확인)
    const manuscriptVisibleStatuses = new Set([
      OrderStatus.GENERATED,
      OrderStatus.AGENCY_REVIEW,
      OrderStatus.COMPLETE,
    ]);
    if (!manuscriptVisibleStatuses.has(order.status)) {
      return {
        ...rest,
        manuscript: null,
      };
