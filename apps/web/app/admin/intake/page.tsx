...
export default function IntakePage() {
  ...
  const [personaOverride, setPersonaOverride] = useState<Record<string, boolean>>({});

  const renderCardActions = (order: Order) => {
    if (order.status === OrderStatus.SUBMITTED) {
      const hasAgencyPersona = Boolean(order.personaId && order.personaSnapshot);
      const showOverride = personaOverride[order.id] ?? false;
      return (
        <div className="space-y-4">
          {/* V2: 영업자가 이미 페르소나 배정한 경우 읽기 전용 표시 */}
          {hasAgencyPersona && (
            <div className="rounded-lg border border-blue-500/30 bg-blue-500/10 p-3">
              <div className="flex items-center justify-between">
                <div>
                  <span className="text-xs font-medium text-blue-300">영업자 지정 페르소나</span>
                  <p className="text-sm mt-0.5" style={{ color: 'var(--text)' }}>{order.personaSnapshot}</p>
                  <span className="text-xs" style={{ color: 'var(--muted)' }}>ID: {order.personaId}</span>
                </div>
                <button
                  type="button"
                  className="btn-outline text-xs"
                  onClick={() => setPersonaOverride((prev) => ({ ...prev, [order.id]: !prev[order.id] }))}
                >
                  {showOverride ? '수정 닫기' : '수정'}
                </button>
              </div>
            </div>
          )}

          {/* 페르소나 미배정이거나 오버라이드 요청 시 기존 배정 UI 표시 */}
          {(!hasAgencyPersona || showOverride) && (
            <>
          <div>
          ...
          <button
            ...
            disabled={assigning === order.id}
          >
            {hasAgencyPersona ? '페르소나 수정' : '페르소나 배정'}
          </button>
            </>
          )}
        ...
        <div className="space-y-6">
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-3">
              {([
                { status: OrderStatus.SUBMITTED, label: '접수', color: '#60a5fa' },
                { status: OrderStatus.ADMIN_INTAKE, label: '인테이크', color: '#a78bfa' },
                { status: OrderStatus.GENERATING, label: '산출중', color: '#fbbf24' },
                { status: OrderStatus.GENERATED, label: '산출완료', color: '#34d399' },
                { status: OrderStatus.ADMIN_REVIEW, label: '검수', color: '#2dd4bf' },
                { status: OrderStatus.FAILED, label: '실패', color: '#f87171' },
                { status: OrderStatus.REGEN_QUEUED, label: '재생성 대기', color: '#fb923c' },
              ] as const).map(({ status, label, color }) => {
                const count = orders.filter((o) => o.status === status).length;
                const withPersona = orders.filter((o) => o.status === status && o.personaId).length;
                return (
                  <div key={status} className="rounded-lg border border-[var(--border)] bg-[var(--bg)] p-3 text-center">
                    <div className="text-lg font-bold" style={{ color }}>{count}</div>
                    <div className="text-xs" style={{ color: 'var(--muted)' }}>{label}</div>
                    {count > 0 && (
                      <div className="text-xs mt-1" style={{ color: withPersona === count ? '#34d399' : '#fbbf24' }}>
                        페르소나 {withPersona}/{count}
                      </div>
                    )}
                  </div>
                );
