...
/* ── V2: 페르소나 프리셋 ─────────────────────────── */
type PersonaPreset = { label: string; personaId: string; snapshot: string };
...
const QUICK_PERSONA_PRESETS: PersonaPreset[] = [
	{ label: '20대 여성 · 활발 · 밝은', personaId: '20-F-FRIENDLY-BRIGHT', snapshot: '20대 여성, 친근하고 밝은 톤의 작성 스타일' },
	{ label: '30대 남성 · 전문 · 중립', personaId: '30-M-PROFESSIONAL-NEUTRAL', snapshot: '30대 남성, 전문적이고 중립적인 톤의 서술' },
	{ label: '40대 여성 · 차분 · 따뜻', personaId: '40-F-CALM-WARM', snapshot: '40대 여성, 차분하고 따뜻한 톤의 설명' },
	{ label: '30대 여성 · 트렌디 · 밝은', personaId: '30-F-TRENDY-BRIGHT', snapshot: '30대 여성, 트렌디하고 밝은 톤의 추천' },
];

interface PersonaSelection { age: string; gender: string; personality: string; tone: string }
/* ───────────────────────────────────────────────── */
...
function NewOrderPageInner() {
	...
	const [videoCount, setVideoCount] = useState(0); // V2: 동영상 플레이스홀더 개수
	// V2: 페르소나 선택
	const [personaMode, setPersonaMode] = useState<'none' | 'preset' | 'custom'>('preset');
	const [selectedPreset, setSelectedPreset] = useState<PersonaPreset | null>(QUICK_PERSONA_PRESETS[0]);
	const [customPersona, setCustomPersona] = useState<PersonaSelection>({ age: '30', gender: 'F', personality: 'FRIENDLY', tone: 'BRIGHT' });
	...
	/* V2: 현재 페르소나 선택 → { personaId, personaSnapshot } | null */
	const resolvePersona = (): { personaId: string; personaSnapshot: string } | null => {
		if (personaMode === 'preset' && selectedPreset) {
			return { personaId: selectedPreset.personaId, personaSnapshot: selectedPreset.snapshot };
		}
		if (personaMode === 'custom') {
			const { age, gender, personality, tone } = customPersona;
			const id = `${age}-${gender}-${personality}-${tone}`;
			const ageLabel = AGE_GROUPS.find((a) => a.value === age)?.label ?? age;
			const genderLabel = GENDERS.find((g) => g.value === gender)?.label ?? gender;
			const persLabel = PERSONALITIES.find((p) => p.value === personality)?.label ?? personality;
			const toneLabel = TONES.find((t) => t.value === tone)?.label ?? tone;
			return { personaId: id, personaSnapshot: `${ageLabel} ${genderLabel}, ${persLabel} ${toneLabel}의 작성 스타일` };
		}
		return null;
	};
	...
	const handleSubmit = async (saveAsDraft = false) => {
		...
		try {
			...
			const payload: OrderCreatePayload = {
				...
				videoCount: videoCount > 0 ? videoCount : undefined, // V2: 동영상 플레이스홀더
				...(() => { const p = resolvePersona(); return p ? { personaId: p.personaId, personaSnapshot: p.personaSnapshot } : {}; })(), // V2: 페르소나
			};
				...
				<div className="max-w-5xl mx-auto">
					<div className="card">
						<div className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_280px]">
							<div>
								...
								<div className="mt-6 space-y-6">
									...
									<div>
										...
										<div className="flex flex-wrap gap-2 mt-2">
											{formData.searchKeywords.map((keyword, index) => (
												<span
													key={`${keyword}-${index}`}
													className="inline-flex items-center gap-1 px-2 py-1 rounded text-xs bg-[var(--border-light)] text-[var(--text)] border border-[var(--border)]"
												>
													{keyword}
													<button
														type="button"
									...
									<div>
										...
										/>
										<p className="text-xs mt-1" style={{ color: 'var(--muted)' }}>
											원고 내에 &quot;동영상 1&quot;, &quot;동영상 2&quot; 등 플레이스홀더가 추가됩니다 (0이면 없음)
										</p>
									</div>
									...
									<div>
												...
												<button
													...
													className={`px-3 py-1 text-xs rounded-full border transition-colors ${
														personaMode === mode
															...
															: 'border-[var(--border)] text-gray-400 hover:border-[var(--border)]'
													}`}
												>
													{mode === 'none' ? '미지정' : mode === 'preset' ? '빠른 선택' : '직접 설정'}
												</button>
											))}
										</div>
										...
										{personaMode === 'preset' && (
											<div className="flex flex-wrap gap-2">
												{QUICK_PERSONA_PRESETS.map((preset) => (
													<button
														key={preset.personaId}
														type="button"
														onClick={() => setSelectedPreset(preset)}
														className={`px-3 py-1.5 text-xs rounded-lg border transition-colors ${
															selectedPreset?.personaId === preset.personaId
																? 'border-blue-500 bg-blue-500/20 text-blue-300'
																: 'border-[var(--border)] text-gray-400 hover:border-[var(--border)]'
														}`}
													>
										...
										{personaMode === 'custom' && (
											<div className="grid grid-cols-2 gap-3">
												<div>
													<span className="block text-xs mb-1" style={{ color: 'var(--muted)' }}>연령대</span>
													<select
												...
												<div>
													<span className="block text-xs mb-1" style={{ color: 'var(--muted)' }}>성별</span>
													<select
														value={customPersona.gender}
														onChange={(e) => setCustomPersona({ ...customPersona, gender: e.target.value })}
														className="input-dark w-full text-sm"
													>
												...
												<div>
													<span className="block text-xs mb-1" style={{ color: 'var(--muted)' }}>성격</span>
													<select
												...
												<div>
													<span className="block text-xs mb-1" style={{ color: 'var(--muted)' }}>톤</span>
													<select
										...
										{personaMode !== 'none' && (
											<p className="text-xs mt-2" style={{ color: 'var(--muted)' }}>
												{personaMode === 'preset' && selectedPreset
													? `✓ ${selectedPreset.snapshot}`
													: personaMode === 'custom'
													? `✓ ${resolvePersona()?.personaSnapshot ?? ''}`
													: '프리셋을 선택해 주세요'}
											</p>
										)}
									</div>
							...
							<aside className="rounded-xl border border-[var(--border)] bg-[var(--bg)] p-4 flex flex-col gap-3">
								...
								{trimmedPlaceName && !templatesLoading && !templatesError && latestTemplate && (
									<div className="space-y-3 text-xs">
										<div className="rounded-lg border border-[var(--border)] bg-[var(--border-light)] p-3 space-y-3">
											...
											<div className="grid gap-2">
												...
												{latestTemplateHashtags.length > 0 && (
													<div className="flex flex-wrap gap-2">
														{latestTemplateHashtags.slice(0, 5).map((tag) => (
															<span key={`hash-${tag}`} className="px-2 py-1 rounded bg-[var(--border-light)] border border-[var(--border)]">
																#{tag}
															</span>
														))}
												...
												{latestTemplateData.notes && (
													<p className="rounded bg-[var(--bg)] p-2" style={{ color: 'var(--muted)' }}>
