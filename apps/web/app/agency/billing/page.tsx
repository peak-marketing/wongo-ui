    ...
    <RouteGuard requiredRole="AGENCY">
        ...
        <div className="p-6 space-y-8">
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-2">
              ...
              <button
                className="px-3 py-2 rounded-md border border-[var(--border)] hover:bg-[var(--bg)]"
                onClick={loadAll}
                title="재조회"
              >
                새로고침
          ...
          <div className="flex items-center justify-between p-4 border-b border-[var(--border)]">
            ...
            <div className="flex items-center gap-3 text-sm">
              ...
              <input
                type="number"
                min={1}
                className="w-20 bg-transparent border border-[var(--border)] rounded px-2 py-1"
                value={topupPage}
                ...
                {(topups?.items || []).map((r) => (
                  <tr key={r.id} className="border-b border-[var(--border-light)]">
                    ...
                    <td className="p-3 text-right">
                      {r.status === 'PENDING' && (
                        <button
                          className="px-3 py-1 rounded border border-[var(--border)] hover:bg-[var(--bg)]"
        ...
        <section className="rounded-lg border border-[var(--border)]">
          <div className="flex items-center justify-between p-4 border-b border-[var(--border)]">
            ...
            <div className="flex items-center gap-2">
              <button className="px-3 py-2 rounded-md border border-[var(--border)] hover:bg-[var(--bg)] text-sm" onClick={exportXlsx}>엑셀로 받기(XLSX)</button>
            </div>
          </div>
          <div className="p-4 grid grid-cols-1 md:grid-cols-4 gap-3 text-sm">
            <select
              className="bg-white text-[var(--text)] border border-[var(--border)] rounded px-2 py-2"
            ...
            <div ref={fromPickerRef} className="relative">
              <input
                ...
                readOnly
                placeholder="시작일"
                className="w-full bg-transparent border border-[var(--border)] rounded px-2 py-2 cursor-pointer"
                value={selectedFromDate ? format(selectedFromDate, 'yyyy-MM-dd', { locale: ko }) : ''}
                onClick={() => setShowFromPicker(!showFromPicker)}
              />
              {showFromPicker && (
                <div className="absolute z-10 mt-1 bg-white border border-[var(--border)] rounded-lg shadow-lg p-3">
                  <DayPicker
                    mode="single"
                  ...
                  />
                  <button
                    className="w-full mt-2 px-3 py-1.5 text-sm border border-[var(--border)] rounded hover:bg-[var(--bg)]"
                    onClick={() => {
            ...
            <input placeholder="최소 금액" className="bg-transparent border border-[var(--border)] rounded px-2 py-2" value={filters.min || ''} onChange={(e)=>setFilters(f=>({...f, min:e.target.value||undefined, page:1}))} />
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
      ...
      {openTopup && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30">
          <div className="w-full max-w-md rounded-lg border border-[var(--border)] bg-white p-6 space-y-4">
            <div className="flex items-center justify-between">
            ...
            <div className="grid grid-cols-5 gap-2">
              {[10_000, 50_000, 100_000, 300_000, 500_000].map((v) => (
                <button key={v} className="px-2 py-2 rounded border border-[var(--border)] hover:bg-[var(--bg)] text-sm" onClick={()=>quickSet(v)}>
            ...
            <div className="flex justify-end gap-2">
              <button className="px-4 py-2 rounded-md border border-[var(--border)]" onClick={()=>setOpenTopup(false)}>취소</button>
              <button
                className="px-4 py-2 rounded-md bg-[var(--brand)] text-white disabled:opacity-60"
